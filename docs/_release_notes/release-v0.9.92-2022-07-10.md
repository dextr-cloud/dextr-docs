...
## ConnectPath Release Notes, July 2022
...
When a new customer signs up for ConnectPath, an extended team at CloudHesive will be notified to better support each customer's individual needs.
...
----
### HAR-127 Allow users to create a support case while logged into ConnectPath (direct customers only):

Customers can now open tickets with our support team from within ConnectPath.
...
Customers may offboard individual instances from ConnectPath now.
...
During sign up, new ConnectPath customers will be prompted to provide their contact information so that we may better support them.
...
----
### HAR-123 Add support for Spanish-language interfaces in ConnectPath; per-user preferences to set preferred defaults:
...
----
### HAR-282 Correct behavior where making an inbound or outbound call results in a notification, in ConnectPath, of 1 new task:

Correct behavior where making an inbound or outbound call results in a notification, in ConnectPath, of 1 new task.

----
### HAR-469 Add support for Spanish-language reports in ConnectPath:
